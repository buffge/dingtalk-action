...
async function run() {
  try {
    ...
    switch (msgtype) {
      ...
      case 'actionCard':
        ...
        if (!isOverAll) {
          ...
          const btnsRes = btnsObj.every(function (item:Record<string,any>) {
      ...
      case 'feedCard':
        ...
        const feedRes = feedObj.every(function (item: Record<string,any>) {
    ...
    let notifyWhenArr = notifyWhen.split(',');

    if (notifyWhenArr.includes(jobStatus)) {
      // send notify
